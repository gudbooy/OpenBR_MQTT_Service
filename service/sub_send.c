...
void my_message_callback(struct mosquitto *mosq, void *obj, const struct mosquitto_message *message)
{
	...
	sprintf(buf, "%d", mytime.tv_sec);
	getcwd(buf1, 100);
	sprintf(path, "%s/%s.jpg",buf1,buf);
	printf("%s",path);
	printf("\n");	
	*/
	...
	if(message->payloadlen){
		snprintf(fileID, 25,  "%s",(char*) message->payload);
	...
	if(cfg->verbose){
		if(message->payloadlen){
			...
			fwrite(message->payload+24, 1, message->payloadlen-24, fp);
			if(cfg->eol){
				printf("\n");
			}
		}else{
